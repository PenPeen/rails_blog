...
RSpec.describe 'PublishedSearchPosts Query', type: :request do
  describe 'published_search_posts' do
    ...
    let!(:apple_post) { create(:post, user: user, title: 'リンゴについて', published: true) }
    let!(:banana_post) { create(:post, user: user, title: 'バナナについて', published: true) }
    let!(:orange_post) { create(:post, user: user, title: 'オレンジについて', published: true) }
    let!(:unpublished_post) { create(:post, user: user, title: 'リンゴのレシピ', published: false) }
